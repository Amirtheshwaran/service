...
public static class CreatureDownload {
 ...
 public static void Run(){Run(new long[]{244853});} public static void Run(long[] ids){
  UnityEditor.PackageManager.UI.Window.Open(ids[0].ToString());
  ...
  managerType.GetMethods(BindingFlags.Instance|BindingFlags.Public).Single(m=>m.Name=="Download").Invoke(manager,new object[]{ids});
